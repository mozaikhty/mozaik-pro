...
window.renderProfileFeed = function() { window.currentGlobalPosts = globalPosts;
    const feedContainer = document.getElementById('profile-feed-container'); 
    ...
    globalPosts.forEach((postObj) => {
        ...
        if (shouldShow) {
            const renderSinglePost = (isPinned) => {
                ...
                if (postData.media && postData.media.length > 1) {
                    let slides = postData.media.map(m => {
                        let tag = m.type === 'video' ? `${window.renderCustomVideo(window.sanitizeUrl(m.url), likeAction, "feed-" + Math.random().toString(36).substr(2,9), postId)}` : `<img src="${window.sanitizeUrl(m.url)}" style="width:100%; max-height:400px; object-fit:cover; border-radius:8px; cursor:pointer;" onclick="${likeAction}">`;
                    ...
                    let m = postData.media[0];
                    let tag = m.type === 'video' ? `${window.renderCustomVideo(window.sanitizeUrl(m.url), likeAction, "feed-" + Math.random().toString(36).substr(2,9), postId)}` : `<img src="${window.sanitizeUrl(m.url)}" class="post-image" style="cursor:pointer;" onclick="${likeAction}">`;
