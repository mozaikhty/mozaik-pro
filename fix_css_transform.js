...
// Replace the previous media query block
css = css.replace(/\/\* MOBİL HEADER COMPACT MODE \*\/[\s\S]*?(?=\n\/\*|$)/g, '');

const newCSS = `/* MOBİL HEADER COMPACT MODE */
@media (max-width: 1000px) {
    /* Header transition - USING TOP INSTEAD OF TRANSFORM FOR STICKY BUGS */
    .header-sticky, .chat-header-main {
        transition: top 0.4s cubic-bezier(0.25, 0.8, 0.25, 1), background 0.3s ease, backdrop-filter 0.3s ease, border-color 0.3s ease !important;
    }
    ...
    .chat-header-main .header-icons {
        transition: opacity 0.3s ease, transform 0.4s cubic-bezier(0.25, 0.8, 0.25, 1) !important;
    }
...
console.log('Fixed CSS appended safely!');
