...
function renderFeed() { window.currentGlobalPosts = globalPosts;
    const feedContainer = document.getElementById('post-alani') || document.getElementById('feed-container'); feedContainer.innerHTML = ''; let displayedPosts = 0;
    globalPosts.forEach(post => {
        ...
        const bookmarkClass = isBookmarked ? 'fa-solid text-cyan-400' : 'fa-regular text-gray-400';
        ...
        if (likeCount > 0) {
            reactionsHtml = `<div class="flex -space-x-1">
                ...
                ${likeCount > 1 && isLiked ? '<span class="z-10 bg-yellow-500 rounded-full w-5 h-5 flex items-center justify-center text-[10px] text-white">👍</span>' : (likeCount > 0 && !isLiked ? '<span class="z-20 bg-red-500 rounded-full w-5 h-5 flex items-center justify-center text-[10px] text-white">❤️</span>' : '')}
            </div>
        ...
        postDiv.innerHTML = `
            <article class="bg-card rounded-2xl p-4 space-y-4" style="background-color: #151e32;">
                ...
                <p class="text-sm text-gray-200 break-words" onclick="window.openPostDetail('${post.id}')">${window.escapeHtml(postData.content || '').replace(/\n/g, '<br>')}</p>
                
                ${customMediaHtml}
                
                <div class="flex justify-between items-center text-gray-400 text-sm mt-3 pt-3">
                    <div class="flex items-center space-x-2 cursor-pointer" onclick="event.stopPropagation(); window.toggleLike('${post.id}', ${isLiked})">
                        ${reactionsHtml}
                        ${likeCount === 0 ? '<i class="fa-regular fa-heart text-gray-400"></i>' : ''}
                    </div>
                    <div class="flex space-x-4">
                        <span class="cursor-pointer" onclick="event.stopPropagation(); window.openPostDetail('${post.id}')"><i class="fa-regular fa-comment text-gray-400"></i> ${commentCount}</span>
                        <span class="cursor-pointer" onclick="event.stopPropagation(); window.openShareModal('${post.id}')"><i class="fa-solid fa-share text-gray-400"></i> ${shareCount}</span>
                        <span class="cursor-pointer" onclick="event.stopPropagation(); window.toggleBookmark('${post.id}', event)"><i class="${bookmarkClass}"></i></span>
                    </div>
                </div>
            </article>
        `;
